...
async function fetchNpmDownloads(repoName) {
  try {
    const response = await fetch(`https://api.npmjs.org/downloads/point/last-month/@${GITHUB_USERNAME}/${encodeURIComponent(repoName)}`);
...
// The GitHub REST API has no unauthenticated way to read pinned status, so pinned
// repo names are kept here and matched against the fetched repository list below.
async function loadPinnedNames() {
  try {
    const response = await fetch("content/pinned.json");
    if (!response.ok) throw new Error("Pinned list request failed");
...
async function loadRepositories() {
  ...
  try {
    const [response, pinnedNames] = await Promise.all([
      fetch(`https://api.github.com/users/${encodeURIComponent(GITHUB_USERNAME)}/repos?type=owner&sort=updated&per_page=100`),
      loadPinnedNames(),
    ]);
    ...
    if (pinnedNames.length) {
      ...
      repositories.sort((first, second) => second.stargazers_count - first.stargazers_count || new Date(second.updated_at) - new Date(first.updated_at));
      selected = repositories.slice(0, 6);
    }

    const npmDownloads = await Promise.all(selected.map((repo) => fetchNpmDownloads(repo.name)));
    selected.forEach((repo, index) => { repo.npmDownloads = npmDownloads[index]; });
...
// Splits "---\nkey: value\n---\nbody" into { data, body }. Not a full YAML parser: one "key: value"
// per line, plus one level of indentation to build a nested object (used for the links map).
...
async function loadBlogPosts() {
  try {
    ...
    posts.sort((first, second) => new Date(second.data.date || 0) - new Date(first.data.date || 0));
    cachedPosts = posts;
    blogCount.textContent = `${posts.length} posts`;
    blogList.innerHTML = posts.map((post) => {
      const tags = (post.data.tags || "").split(",").map((tag) => tag.trim()).filter(Boolean);
...
async function loadContact() {
  try {
    ...
    const links = data.email ? [["Email", `mailto:${data.email}`], ...Object.entries(data.links || {})] : Object.entries(data.links || {});
    ...
    contactContent.innerHTML = `
      ...
      <ul class="contact-links">
        ...
        ${data.availability ? `<li class="contact-meta">${data.availability}</li>` : ""}
      </ul>
    `;
  } catch (error) {
